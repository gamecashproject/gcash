...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: gamecash.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: gamecashd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 58754 or testnet: 45554)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 58854 or testnet: 45454)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeGamecashamount=<n> " + _("Keep N Gamecash anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    int nUserMaxConnections = GetArg("-maxconnections", 192);
    if (nUserMaxConnections > FD_SETSIZE) {
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Gamecash is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Gamecash is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Gamecash version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeGamecashAmount = GetArg("-anonymizeGamecashamount", 0);
    if(nAnonymizeGamecashAmount > 999999) nAnonymizeGamecashAmount = 999999;
    if(nAnonymizeGamecashAmount < 2) nAnonymizeGamecashAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeGamecashAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize Gamecash Amount %d\n", nAnonymizeGamecashAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1Gamecash+1000 == (.1Gamecash+100)*10
       10Gamecash+10000 == (1Gamecash+1000)*10
    */
