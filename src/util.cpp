...
int nAnonymizeGamecashAmount = 500;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Gamecash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Gamecash
    // Mac: ~/Library/Application Support/Gamecash
    // Unix: ~/.Gamecash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Gamecash";
#else
    fs::path pathRet;
...
void createConf()       //Automatic gamecash.conf generation
{
    srand(time(NULL));
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "gamecash.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "gamecashd.pid"));
