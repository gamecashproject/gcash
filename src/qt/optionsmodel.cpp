...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeGamecashAmount"))
        settings.setValue("nAnonymizeGamecashAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeGamecashAmount = settings.value("nAnonymizeGamecashAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeGamecashAmount"))
        SoftSetArg("-anonymizeGamecashamount", settings.value("nAnonymizeGamecashAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeGamecashAmount:
            nAnonymizeGamecashAmount = value.toInt();
            settings.setValue("nAnonymizeGamecashAmount", nAnonymizeGamecashAmount);
            emit anonymizeGamecashAmountChanged(nAnonymizeGamecashAmount);
            break;
        default:
            break;
        }
