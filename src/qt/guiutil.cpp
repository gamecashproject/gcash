...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Gamecash"))
        return false;
...
{
    // Convert Gamecash:// to Gamecash:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Gamecash://"))
    {
        uri.replace(0, 12, "Gamecash:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Gamecash.lnk";
}
...
{
    return GetAutostartDir() / "Gamecash.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Gamecash-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Gamecash-qt [" + tr("command-line options") + "]                     " + "\n";
